...
public class ClusterService {
    ...
    public List<ClusterResponse> getAllClusters() {
        return clusterRepository.findAll().stream().map(this::mapToResponse).toList();
    }
    ...
    @Transactional(readOnly = true)
    public List<ClusterResponse> getMyClusters() {
        User manager = currentUserService.getCurrentEnabledUser();
        return clusterRepository.findByManagerId(manager.getId()).stream().map(this::mapToResponse).toList();
    }
    ...
    public void deleteCluster(Long id) {
        User manager = currentUserService.getCurrentEnabledUser();
        BadmintonCluster cluster = getManagedCluster(id, manager.getId());
        if (bookingRepository.existsByCourtClusterId(id)) {
